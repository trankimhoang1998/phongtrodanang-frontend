...
const postDetail = computed(() => {
  return postStore.postDetail
})

const getPostDetail = async () => {
  const id = route.params.id
  await postStore.getPostDetail(id as string)
}
...
<template>
  <PageWrapper>
    <PageHeader>
      <PageTitle :text="`Post detail`" class="capitalize" />
    </PageHeader>
    <PageBody class="lg:w-3/4">
      <PageSection>
        <div
          class="dark:bg-gray-900 flex items-center justify-center"
            ...
            <div class="flex justify-between items-center mt-5">
              <div class="flex">
                ...
                <span
                  ...
                  >{{ postDetail?.like_count }}</span
                >
              </div>
              <div class="ml-1 text-gray-500 dark:text-gray-400 font-light">
                {{ postDetail?.comment_count }} comments
              </div>
            </div>
          </div>
        </div>
